...
def prep_smpl_to_qpos_expert(smpl_output_folder, ori_amass_root_folder):
    ...
    model_file = f'kinpoly/assets/mujoco_models/humanoid_smpl_neutral_mesh.xml'
    humanoid_model =load_model_from_path(model_file)

    # Convert the data format 
    smpl_seq_data = {} 
    # smpl_output_folder = "/move/u/jiamanli/datasets/egoego_processed_data/amass_same_shape_kinpoly_format_data"
    if not os.path.exists(smpl_output_folder):
        os.makedirs(smpl_output_folder)
    smpl_output_filename = os.path.join(smpl_output_folder, "amass_kinpoly_motion.p")

    # ori_amass_root_folder = "/move/u/jiamanli/datasets/egoego_processed_data/processed_amass_same_shape" 

    # TEST_DATASETS = ['MPI_HDM05', 'SFU', 'MPI_mosh', 'Transitions_mocap', 'HumanEva'] # HuMoR test datasets
    ...
    for subset_name in subset_folders:    
        ...
        for seq_name in seq_folders:
            ...
            for npz_name in npz_files:
                ...
                dest_seq_name = subset_name+"-"+seq_name+"-"+npz_name.replace(".npz", "")
...
def reorganize_data(ori_data_file, dest_data_folder):
    ...
    for seq_name in data_dict:
        ...
        elif seq_name.split("-")[0] in VAL_DATASETS:
            new_meta_dict['val'].append(seq_name)
        elif seq_name.split("-")[0] in TEST_DATASETS:
...
if __name__ == "__main__":
    smpl_output_folder = "data/amass_processed_for_kinpoly"
    ori_amass_data_folder = "data/processed_amass_same_shape_seq"
    prep_smpl_to_qpos_expert(smpl_output_folder, ori_amass_data_folder) 

    # Convert the data to the same organized folder as original kinpoly data 
    ori_data_file = os.path.join(smpl_output_folder, "amass_kinpoly_motion.p")
    dest_data_folder = os.path.join(smpl_output_folder, "MoCapData")
    template_meta_file = "data/kinpoly-mocap/mocap_meta.yml"
    reorganize_data(ori_data_file, dest_data_folder, template_meta_file)
