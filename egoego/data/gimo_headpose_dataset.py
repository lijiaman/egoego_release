...
class GIMOHeadPoseDataset(Dataset):
    def __init__(
        self,
        data_root_folder, 
        train: bool,
        ...
        self.head_data_path = os.path.join(data_root_folder, "gimo_processed_for_kinpoly/MoCapData/features/mocap_annotations.p")
        ...
        slam_res_folder = os.path.join(self.data_root_folder, "gimo/droid_slam_res") # 2 is better than 1 and each calib 
        for tmp_k in self.data_dict:
            scene_name = self.data_dict[tmp_k]['seq_name'].split("-")[0]
            
            npy_name = "-".join(self.data_dict[tmp_k]['seq_name'].split("-")[1:])
            npy_name = "_".join(npy_name.split("_")[:-4])
            ...
            if os.path.exists(slam_seq_npy):
                ...
                self.load_slam_res_and_align_first(slam_seq_npy, self.data_dict[tmp_k]['head_pose'])
    ...
    def divide_train_val(self):
        train_data_path = self.head_data_path.replace("mocap_", "window_"+str(self.window)+"_train_mocap_")
        val_data_path = self.head_data_path.replace("mocap_", "window_"+str(self.window)+"_val_mocap_")

        of_folder = os.path.join(self.data_root_folder, "gimo/raft_of_feats")
        scene_names = os.listdir(of_folder)

        val_scene_names = ["storeroom0217", "classroom0219", "lab0220", "kitchen0214"]
        train_scene_names = []
        for tmp_scene in scene_names:
            if tmp_scene not in val_scene_names:
                train_scene_names.append(tmp_scene)
            
        # Prepare train val split for seq names. 
        train_val_split_json = self.head_data_path.replace("mocap_annotations.p", "train_val_seq_split.json")
        if not os.path.exists(train_val_split_json):
            ...
            for s_name in seq_names:
                curr_scene_name = s_name.split("-")[0]
                if curr_scene_name in train_scene_names:
                    train_seq_names.append(s_name)

                if curr_scene_name in val_scene_names:
        ...
        if not os.path.exists(train_data_path):
            ...
            for s_name in train_seq_names:
                ...
                if curr_seq_len > self.window + 1:
                    ...
                    curr_of_files = curr_seq_data['of_files']
    ...
    def load_of_feats(self, of_files):
        ofs = []
        for of_file in of_files:
            of_file = of_file.replace("/move/u/jiamanli/datasets/gimo_processed/", os.path.join(self.data_root_folder, "gimo/"))
            of_i = np.load(of_file)
            ofs.append(of_i)
    ...
    def __getitem__(self, index):
        ...
        query['seq_len'] = window_of_data.shape[0]
