...
def quaternion_matrix(_q):
    ...
    return torch.tensor([
        ...
        [    q[1, 3]-q[2, 0],     q[2, 3]+q[1, 0], 1.0-q[1, 1]-q[2, 2]]], dtype=dtype, device=device)
...
def quaternion_about_axis(angle, axis):
    ...
    # q = torch.cat((torch.cos(angle/2.0), q[1], q[2], q[3]), dim=0) 
    return q
...
if __name__ == "__main__":
    ...
    from relive.utils.math_utils import quaternion_matrix as quaternion_matrix_np
    from relive.utils.transformation import quaternion_inverse as quaternion_inverse_np
    from relive.utils.math_utils import transform_vec as transform_vec_np
    from relive.utils.math_utils import quat_from_expmap as quat_from_expmap_np
    from relive.utils.transformation import quaternion_multiply as quaternion_multiply_np
    from relive.utils.math_utils import get_heading_q as get_heading_q_np
    from relive.utils.math_utils import get_heading as get_heading_np
    from relive.utils.transformation import rotation_from_quaternion as rotation_from_quaternion_np
    from relive.utils.math_utils import quat_mul_vec as quat_mul_vec_np
    from relive.utils.math_utils import de_heading as de_heading_np
    ...
    for d in devices:
        ...
        ex_quat_batch    = torch.tensor([[  0.785, 1.360, 0.197, 0.464],
                                        [  1.777, 3.363, 0.363, 0.363],
                                        [  1.522, 0.560, 0.161, 0.623]], dtype=torch.float64)
        ex_quat_np = np.array    ([  1.522, 0.560, 0.161, 0.623], dtype=np.float64)
        ex_quat_batch_np    = np.array([[  0.785, 1.360, 0.197, 0.464],
                                        [  1.777, 3.363, 0.363, 0.363],
                                        [  1.522, 0.560, 0.161, 0.623]], dtype=np.float64)
