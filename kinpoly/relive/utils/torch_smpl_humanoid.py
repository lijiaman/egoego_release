...
class Humanoid:
    ...
    def get_body_occup_map(self, qpos, body_name):
        ...
        body_pos, body_quat = self.qpos_fk(torch.unsqueeze(qpos, dim=0), select_joints = body_name)
        ...
        grid_h = torch.cat((grid.view(tot_num, 3).t(), torch.ones(1, tot_num, dtype=dtype, device=device)), dim=0)
        ...
        occup_grid_batch = cond.all(dim=2).any(dim=0).view(occup_grid_size).type(dtype).to(device)
    ...
    def qpos_fk(self, qpos, select_joints=None, fix_quat = True):
        ...
        if fix_quat:
            ...
            qpos = torch.cat([qpos[:, :3], root_rotations, qpos[:, 7:]], dim = 1) # had to do this for gradient flow
        ...
        body_quats = quaternion_from_euler(padded_rot[:, 0], padded_rot[:, 1], padded_rot[:, 2], axes='rzyx')
        body_quats = body_quats.reshape(B, J, 4)
        
        wbody_pos, body_com, wbody_quat = self.forward_kinematics_batch(body_quats, root_rotations, root_positions)

        ''' Get sim full vals'''
        body_quats_full = quaternion_from_euler(padded_rot[:, 0], padded_rot[:, 1], padded_rot[:, 2], axes = "sxyz")
        body_quats_full = torch.cat((root_rotations[:, None, :], body_quats_full.reshape(B, J, 4)), dim = 1)
    ...
    def forward_kinematics_batch(self, rotations, root_rotations, root_positions):
        ...
        positions_world_com = []
        rotations_world = []
        expanded_offsets = self._offsets.expand(B, J, self._offsets.shape[1]).to(device).type(dtype)
        expanded_i_offsets = self._i_offsets.expand(B, J, self._i_offsets.shape[1]).to(device).type(dtype)
        

        for i in range(J):
            if self._parents[i] == -1:
                positions_world.append(root_positions)
                positions_world_com.append(root_positions + quat_mul_vec_batch(root_rotations, expanded_i_offsets[:, 0, :]))
                rotations_world.append(root_rotations)
            else:
                jpos = quat_mul_vec_batch(rotations_world[self._parents[i]], expanded_offsets[:, i, :]) +  positions_world[self._parents[i]]
                jquat = quaternion_multiply_batch(rotations_world[self._parents[i]], rotations[:, i - 1, :])

                positions_world.append( jpos)
                rotations_world.append(jquat)
                positions_world_com.append(quat_mul_vec_batch(jquat, expanded_i_offsets[:, i, :]) + jpos) 
        ...
        return positions_world.permute(0, 2, 1), positions_world_com.permute(0, 2, 1), rotations_world.permute(0, 2, 1)

    def forward_kinematics_batch_w_rot_mat(self, rotations, root_rotations, root_positions):
        """
        Perform forward kinematics using the given trajectory and local rotations.
        Arguments (where B = batch size, J = number of joints):
         -- rotations: (B, J, 3, 3) tensor of unit quaternions describing the local rotations of each joint.
         -- root_rotations: (B, 3, 3)
         -- root_positions: (B, 3) tensor describing the root joint positions.
        Output: joint positions (B, J, 3), gloabl_joint_rotations: (B, J, 3, 3)
        """ 
        ...
        for i in range(J):
            if self._parents[i] == -1:
                positions_world.append(root_positions) # B X 3 
                rotations_world.append(root_rotations) # B X 3 X 3 
            else:
                # jpos = quat_mul_vec_batch(rotations_world[self._parents[i]], expanded_offsets[:, i, :]) +  positions_world[self._parents[i]]
                # jquat = quaternion_multiply_batch(rotations_world[self._parents[i]], rotations[:, i - 1, :])
                jpos = torch.matmul(rotations_world[self._parents[i]], expanded_offsets[:, i, :][:, :, None]).squeeze(-1) +  \
                    positions_world[self._parents[i]] # B X 3 X 1
                jrot = torch.matmul(rotations_world[self._parents[i]], rotations[:, i - 1, :, :]) # B X 3 X 3 

                positions_world.append(jpos)
                rotations_world.append(jrot)

        positions_world = torch.stack(positions_world, dim=1) # B X J X 3
        rotations_world = torch.stack(rotations_world, dim=1) # B X J X 3 X 3 
...
def get_expert(expert_qpos):
    ...
    qpos  = torch.tensor(expert_qpos, dtype=dtype, device=device)
    fk_res = env_humanoid.qpos_fk(qpos)
    

    print(np.sum(np.abs(wbody_pos_mujoco - fk_res['wbpos'].cpu().numpy())), \
          np.sum(np.abs(wbody_quat_mujoco - fk_res['wbquat'].cpu().numpy())), \
          np.sum(np.abs(body_quat_mujoco - fk_res['bquat'].cpu().numpy())), \
          np.sum(np.abs(body_com_mujoco - fk_res['body_com'].cpu().numpy())), \
        )
...
if __name__ == "__main__":
    ...
    cfg = Config("copycat_5", "/insert_directory_here//", create_dirs=False, \
            mujoco_path = "%s.xml"
        )
