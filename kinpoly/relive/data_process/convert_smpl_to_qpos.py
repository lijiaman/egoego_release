...
def prep_smpl_to_qpos_expert():
    ...
    smpl_seq_data = {} 
    smpl_output_folder = "/viscam/u/jiamanli/datasets/egomotion_syn_dataset/locomotion_data_for_kinpoly"
    if not os.path.exists(smpl_output_folder):
        os.makedirs(smpl_output_folder)
    smpl_output_filename = os.path.join(smpl_output_folder, "locomotion_subset_motion.p")

    # data_folder = "/viscam/u/jiamanli/datasets/egomotion_syn_dataset/habitat_rendering_replica_locomotion/apartment_0/CMU_91_91_10_poses_634_frames_30_fps_b100seq0_samp_0"
    data_root_folder = "/viscam/u/jiamanli/datasets/egomotion_syn_dataset/habitat_rendering_replica_locomotion"
    ...
    scene_name_list = [] # 18 Replica scenes 
    scene_obj_folder = "/viscam/u/jiamanli/datasets/replica_processed/replica_fixed_poisson_recon_objs"
    scene_objs = os.listdir(scene_obj_folder)
    for scene_name in scene_objs:
        scene_name_list.append(scene_name.replace(".obj", ""))

    for scene_name in scene_name_list:
        scene_folder = os.path.join(data_root_folder, scene_name)
        seq_folders = os.listdir(scene_folder)
        for seq_name in seq_folders:
            motion_path = os.path.join(scene_folder, seq_name, "motion_seq.npz")
            if os.path.exists(motion_path):
                npz_data = np.load(motion_path)

                start_frame_idx = int(npz_data['start_frame_idx'])
                num_used_frames = npz_data['trans'].shape[0]

                total_frame_cnt += num_used_frames

                ori_amass_path = os.path.join(ori_amass_root_folder, str(npz_data['path'][0])) 
                ori_npz_data = np.load(ori_amass_path) 

                amass_root_pose = ori_npz_data['root_orient'][start_frame_idx:start_frame_idx+num_used_frames] # T X 3 
                amass_pose = ori_npz_data['pose_body'][start_frame_idx:start_frame_idx+num_used_frames] # T X 63 
                amass_trans = ori_npz_data['trans'][start_frame_idx:start_frame_idx+num_used_frames] # T X 3 
                ...
                seq_name = str(npz_data['path'][0]).replace("/", "-")
...
if __name__ == "__main__":
    ...
    ori_data_file = "/viscam/u/jiamanli/datasets/egomotion_syn_dataset/locomotion_data_for_kinpoly/locomotion_subset_motion.p"
    dest_data_folder = "/viscam/u/jiamanli/datasets/egomotion_syn_dataset/locomotion_data_for_kinpoly/MoCapData"
