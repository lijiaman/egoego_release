...
def prep_smpl_to_qpos_expert():
    ...
    smpl_output_folder = "/viscam/u/jiamanli/datasets/kinpoly_processed_for_kinpoly_new"
    if not os.path.exists(smpl_output_folder):
        os.makedirs(smpl_output_folder)
    smpl_output_filename = os.path.join(smpl_output_folder, "humor_processed_kinpoly_motion.p")
    ...
    ori_amass_root_folder = "/viscam/u/jiamanli/datasets/kin-poly/processed_kinpoly_for_humor_new"

    total_frame_cnt = 0
    counter = 0

   
    seq_files = os.listdir(ori_amass_root_folder)
    for seq_name in seq_files:
        if ".npz" in seq_name:
            motion_path = os.path.join(ori_amass_root_folder, seq_name) 
            ...
            dest_seq_name = "_".join(seq_name.split("_")[:3]) 
...
def reorganize_data(ori_data_file, dest_data_folder):
    ...
    # Set train and test 
    new_meta_dict['train'] = meta_list['train']
    # new_meta_dict['val'] = meta_list['val']
    new_meta_dict['test'] = meta_list['test']
...
if __name__ == "__main__":
    ...
    ori_data_file = "/viscam/u/jiamanli/datasets/kinpoly_processed_for_kinpoly_new/humor_processed_kinpoly_motion.p"
    dest_data_folder = "/viscam/u/jiamanli/datasets/kinpoly_processed_for_kinpoly_new/MoCapData"
    reorganize_data(ori_data_file, dest_data_folder)

    # Total number of sequences:266
    # Total number of frames:147512
