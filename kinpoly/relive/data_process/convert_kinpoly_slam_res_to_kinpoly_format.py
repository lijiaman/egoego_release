...
def gen_res_for_infer(meta_file, data_p_path, slam_res_folder, dest_data_p_path):
    dest_data_dict = {} 
    data = joblib.load(data_p_path)

    meta_list = yaml.load(open(meta_file, 'r'), Loader=yaml.FullLoader)
    video_mocap_sync_dict = meta_list['video_mocap_sync']

    test_key_list = meta_list['test']
    ...
    for seq_name in data:  
        ...
        if os.path.exists(slam_res_path):
            ...
            slam_head_pose = slam_data[lb + im_offset: ub + im_offset]
...
if __name__ == "__main__":
    ...
    # Kinpoly data after processing using HuMoR script, adjust the height to be consistent with the AMASS dataset 
    ...
    slam_res_folder = "/viscam/u/jiamanli/datasets/kin-poly/OriData/droid_slam_res"
    dest_data_folder = "/viscam/u/jiamanli/datasets/kin-poly/humor_processed_kinpoly_slam_res_for_s2/MoCapData/features"
    ...
    gen_res_for_infer(dest_meta_path, data_p_path, slam_res_folder, dest_data_p_path)
