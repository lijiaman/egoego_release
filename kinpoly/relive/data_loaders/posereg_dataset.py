...
class PoseRegDataset(data.Dataset):
    ...
    def preprocess_data(self, data_mode = "train"):
        ...
        for take in tqdm(self.takes):
            ...
            "1205_take" in take or "1219_take_47" in take or "1213_take_58" in take or \
            "1213_take_56" in take: 
                continue
                
            gt_qpos = gt_qpos[:len(of_files)]
            seq_len = gt_qpos.shape[0]


            
            if self.data_mode == "train" and seq_len <= self.fr_num and not self.cfg.wild:
            # if seq_len <= self.fr_num and not self.cfg.wild:
                continue
            ...
            # target_qvel = curr_expert['qvel'] # Previous a bug here using this!!!
            target_qvel = self.get_traj_vel(gt_qpos)

            # Added by Jiaman 
            posereg_traj_pos = self.get_traj_pos(gt_qpos)
            posereg_traj = np.hstack((posereg_traj_pos, target_qvel))

            data_all['posereg_traj'].append(posereg_traj)
            ...
            # if curr_expert['head_vels'].shape[0] != seq_len:
            #     curr_expert['head_vels'] = curr_expert['head_vels'][:-1]
            #     curr_expert['head_pose'] = curr_expert['head_pose'][:-1]
        ...
        # Added by Jiaman ,follow PoseReg, normalize trajectory 
        all_posereg_traj = data_all['posereg_traj']
        all_traj = np.vstack(all_posereg_traj)
        self.mean = np.mean(all_traj, axis=0)
        self.std = np.std(all_traj, axis=0)
        # np.save("./tmp_mean.npy", self.mean)
        # np.save("./tmp_std.npy", self.std)
       
        # data_all['posereg_traj_norm'] = all_posereg_traj # v2, v3
        data_all['posereg_traj_norm'] = self.normalize_traj(all_posereg_traj) # v1, v4
        print("Normalize traj data, mean :{0}".format(self.mean.shape))
    ...
    def load_of_feats(self, of_files):
        ofs = []
        for of_file in of_files:
            curr_of_path = of_file.replace("/viscam/u/jiamanli/datasets/kin-poly/ReliveDatasetRelease/fpv_raft_of_feats", \
            "/move/u/jiamanli/datasets/kin-poly/ReliveDatasetRelease/fpv_of_feats")
            curr_of_path = curr_of_path.replace("/viscam/u/jiamanli/datasets/egomotion_syn_dataset", \
            "/viscam/projects/egoego/datasets/ares").replace("raft_flows", "raft_of_feats")
            of_i = np.load(curr_of_path)
            ofs.append(of_i)
    ...
    def get_sample_from_take_ind(self, take_ind, full_sample = False):
        ...
        if full_sample:
            ...
            if self.data["qpos"][take_ind].shape[0] - self.fr_num <= 0:
                ...
                # self.fr_start = fr_start = 0 # For debug 
    ...
    def sampling_generator(self, batch_size=8, num_samples=5000, num_workers=1, fr_num = 80):
        ...
        # self.sample_indices = [0] * num_samples # For debug 
