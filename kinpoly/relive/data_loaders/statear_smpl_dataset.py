...
class StateARDataset(data.Dataset):
    ...
    def preprocess_data(self, data_mode = "train"):
        ...
        for take in tqdm(self.takes):
            if take not in self.data_traj:
                take_cnt += 1 
                continue # For SLAM results in amass-syn , some not exists droid slam results 
    ...
    def load_of_feats(self, of_files):
        ofs = []
        for of_file in of_files:
            of_i = np.load(of_file.replace("raft_flows", \
            "raft_of_feats").replace("/viscam/u/jiamanli/datasets/egomotion_syn_dataset/habitat_rendering_replica_all", \
            "/viscam/projects/egoego/datasets/ares/habitat_rendering_replica_all"))
            ofs.append(of_i)
        ofs = np.stack(ofs) # T X D 
