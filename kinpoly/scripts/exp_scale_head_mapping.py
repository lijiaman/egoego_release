...
from relive.models.head_scale_transformer import HeadScaleTransformer 
...
if __name__ == "__main__":
    ...
    traj_ar_net = HeadScaleTransformer(cfg, device=device, dtype=dtype, mode=args.mode)
    if args.iter > 0:
    ...
    if args.mode == 'train':
        ...
        for i_epoch in range(args.iter, cfg.num_epoch):
            ...
            generator = dataset.sampling_generator(num_samples=cfg.num_sample, \
                batch_size=cfg.batch_size, num_workers=2, fr_num=fr_num)
        ...
        with torch.no_grad():
            if vis_res:
                ...
                # dest_root_folder = "/viscam/u/jiamanli/datasets/egomotion_syn_dataset/head_scale_estimator_res"
                # dest_root_folder = "/viscam/u/jiamanli/datasets/gimo_processed/head_estimator_res"
                dest_root_folder = "/viscam/u/jiamanli/datasets/kin-poly/seq_50_head_scale_estimator_res"

                eval_kinpoly = True 
                eval_syn_amass = False   
                tmp_dict = {}
                for seq_key, data_dict in cur_jobs:
                    ...
                    head_trans_scale = val_feature_pred['seq_dist'] # B X T
                    ...
                    if eval_syn_amass:
                        ...
                        dest_npy_path = os.path.join(dest_root_folder, seq_key.split("-")[1]+".npy")
                    ...
                    np.save(dest_npy_path, head_trans_scale[0].data.cpu().numpy()) # T 
                    print("Saving:{0}".format(dest_npy_path))
