...
if __name__ == "__main__":
    ...
    traj_ar_net = HeadMappingTransformer(cfg, device=device, dtype=dtype, mode=args.mode)
    if args.iter > 0:
    ...
    if args.mode == 'train':
        ...
        with torch.no_grad():
            if vis_res:
                ...
                dest_root_folder = "/viscam/u/jiamanli/datasets/kin-poly/seq_50_head_estimator_res"
                eval_kinpoly = True 
                ...
                for seq_key, data_dict in cur_jobs:
                    ...
                    print("Current seq takes time:{0}".format(interval)) 
                    time_list.append(interval)
                    time_cnt += 1

                    if time_cnt >= 100:
                        avg_time = np.asarray(time_list).mean()
                        print("Avg time:{0}".format(avg_time))
                        import pdb 
                        pdb.set_trace() 
                        break 
                    ...
                    # print("Save:{0}".format(dest_npy_path))
